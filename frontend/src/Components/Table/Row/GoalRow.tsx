...
const GoalRow = ({ goal }: { goal: Goal }) => {
    ...
    const handleDelete = async () => {
        if (goal === null) return;
        confirm({ description: "Are you sure you want to delete this goal?" }).then(async () => {
    ...
    return (
        <>
            ...
            {edit && <EditGoalModal open={edit} handleClose={() => setEdit(false)} goal={editedGoal} updateGoal={editGoal} />}
        </>
    );
};
